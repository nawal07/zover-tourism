        ...
        <div className="grid gap-6 md:grid-cols-3">
          {aboutMilestones.map((m, i) => (
            <MotionWrapper key={m.year} delay={i * 0.1}>
              <article className="rounded-xl border border-border bg-pearl p-6 text-center">
                <p className="font-display text-4xl text-gold">{m.year}</p>
                <h3 className="mt-3 font-heading text-lg text-midnight">
      ...
      <SectionWrapper>
        ...
        <ul className="mt-8 grid gap-4 sm:grid-cols-3">
          {coreValues.map((v) => (
            <li
              key={v.id}
              className="rounded-lg bg-sand px-5 py-4 text-center"
            >
        ...
        <ul className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {targetSegments.map((seg, i) => (
            <MotionWrapper key={seg.id} delay={i * 0.06}>
              <li className="h-full rounded-xl border border-border bg-pearl p-6">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2">
          {targetCustomers.map((c, i) => (
            <MotionWrapper key={c.title.en} delay={i * 0.1}>
              <article
                className={cn(
                  "rounded-xl border border-gold/20 bg-pearl p-8",
          ...
          {homeStrengths.map((s, i) => (
            <MotionWrapper key={s.id} delay={i * 0.08}>
              <li className="flex gap-4 rounded-lg border border-border p-5">
                ...
                <div>
                  <h3 className="font-heading text-midnight">
